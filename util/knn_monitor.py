    ...
    with torch.no_grad():
        ...
        for k,(data, target) in enumerate(memory_data_loader):
            ...
            if vit_backbone==1:
                ...
                feature,_,_ = net.module.forward_encoder(data.cuda(non_blocking=True),0)
                feature = feature[:,0]#torch.mean(feature,dim=1)
            # elif vit_backbone==3:
            #     feature = net(data.cuda(non_blocking=True),return_feature=True)
            #     feature = feature[:,0]
            else:
        ...
        for  k,(data, target) in test_bar:
            ...
            if vit_backbone==1:
                ...
                feature,_,_ = net.module.forward_encoder(data,0)
                feature = feature[:,0]#torch.mean(feature,dim=1)
            # elif vit_backbone==3:
            #     feature = net(data,return_feature=True)
            #     feature = feature[:,0]
            else:
                feature = net(data)
                if pool_ops:
